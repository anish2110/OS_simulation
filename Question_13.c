...
#define MAX_CUSTOMERS 10 // Function prototypes...
...
sem_t waitingRoom; // waitingRoom Limits the number of customers allowed  to enter the waiting room at one time.
sem_t barberChair; // barberChair ensures mutually exclusive access to the barber chair.
...
int allDone = 0; // Flag to stop the barber thread when all customers have been serviced.
...
int main(int argc, char *argv[]) 
{
    ...
    printf("Maximum number of customers can only be 10. Enter number of customers and chairs.\n");
    scanf("%d",&x);
    numCustomers = x; 
    scanf("%d",&x);
    numChairs = x;
    ...
    pthread_create(&btid, NULL, barber, NULL); // Create the barber. 
    ...
    allDone = 1; // When all of the customers are finished, kill the barber thread.
